...
class ProviderManager {
  ...
  async initialize(configs = {}) {
    ...
    const fallbackMode = configs.fallbackMode === true || 
    ...
    this.providers.set("adsgram-cpc", new AdsgramCPC({ ...defaultConfig, ...(configs["adsgram-cpc"] || {}) }));
    this.providers.set("adsgram-cpm", new AdsgramCPM({ ...defaultConfig, ...(configs["adsgram-cpm"] || {}) }));

    if (fallbackMode) {
      console.log("🔧 [ProviderManager] Fallback режим включен - используются тестовые данные");
    }
  ...
  async startAd(providerName) {
    ...
    try {
      ...
      console.error(`[ProviderManager] Ошибка запуска рекламы через ${providerName}:`, error);
      return {
        success: false,
        cancelled: true,
        noAd: false,
  ...
  async startAdWithFallback(actionType = "view", preferredProvider = null) {
    ...
    if (preferredProvider) {
      ...
      if (provider) {
        ...
        if (isAvailable) {
          console.log(`[ProviderManager] Используем предпочтительный провайдер: ${normalizedName}`);
    ...
    for (const providerName of providerList) {
      ...
      try {
        ...
        const isAvailable = await provider.isAdAvailable();
        if (isAvailable) {
          console.log(`[ProviderManager] Найден доступный провайдер: ${providerName}`);
          return await this.startAd(providerName);
        }
      } catch (error) {
        console.error(`[ProviderManager] Ошибка проверки провайдера ${providerName}:`, error);
        continue;
      }
    }
