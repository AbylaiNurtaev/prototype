...
class Adextra extends Provider {
  ...
  async loadSDK() {
    ...
    while (!window.Adextra && attempts < maxAttempts) {
      await new Promise((resolve) => setTimeout(resolve, 100));
      attempts++;
    }

    if (!window.Adextra) {
      console.warn("[Adextra] SDK не загружен. Убедитесь, что скрипт добавлен в index.html");
    }
  }
  ...
  async checkAdAvailability() {
    if (!this.sdk || !window.Adextra) {
      return false;
    }

    try {
      return await this.sdk.isAdReady();
    } catch (error) {
      console.error("[Adextra] Ошибка проверки доступности:", error);
      return false;
    }
  }
  ...
  async fetchAd() {
    ...
    try {
      ...
      return {
        ...
        image_url: ad.imageUrl || ad.bannerUrl || "",
        link: ad.clickUrl || ad.targetUrl || "",
        provider: this.name,
      };
    } catch (error) {
      console.error("[Adextra] Ошибка загрузки рекламы:", error);
      throw error;
    }
  }
